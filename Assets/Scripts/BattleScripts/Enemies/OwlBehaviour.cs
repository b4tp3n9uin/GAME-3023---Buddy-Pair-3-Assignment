...
public class OwlBehaviour : AICharacter
{
    protected override Ability ChooseMove()
    {
        ...
        // If our Owl's Health is above 60, they are more likely to use their first ability and cannot use their heal
        if (currentHealth > 60)
        {
            weights[0] = 0.8f;
            weights[1] = 0.2f;
        ...
        // If Owl's health is less that 40, more likely to look for food, less likely to scratch
        if (currentHealth < 40)
        {
            weights[0] = 0.1f;
            weights[1] = 0.3f;
